...
class ScholarlyDataFetcher(GeneralDataFetcher):
    ...
    def __init__(self, proxy: bool = False):
        ...
        if self.proxy_enabled:
            ...
            logging.info("Proxy enabled using ProxyGenerator.")
    ...
    def get_new_data_since(self, query: str, date: datetime) -> List[Dict]:
        ...
        filename = f"{query}_{datetime.today().isoformat()}.json"
    ...
    def generate_all_queries(self, base_query: str, additional_terms: List[str]) -> List[str]:
        ...
        filename = f"{base_query}_{datetime.today().isoformat()}.json"
        with open(filename, 'w') as file:
    ...
    def dispatch_scholarly_requests(queries: List[str]):
        ...
        for query in queries:
            message = GetGoogleScholarData(ScholarlyDataFetcher.INTERFACE_ID, query)
